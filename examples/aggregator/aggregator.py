...
In the example 'result.py', we discussed the *Results* object, which contains information on the *non-linear search*
...
# %%
"""
For each dataset we are going to set up its outputp path, create its mask and fit it with the phase above. 

The 3 datasets will come from the 'autofit_workspace/complex/simulator.py' module.
...
datas = [simulator.data_0, simulator.data_1, simulator.data_2]
noise_maps = [simulator.noise_map_0, simulator.noise_map_1, simulator.noise_map_2]
